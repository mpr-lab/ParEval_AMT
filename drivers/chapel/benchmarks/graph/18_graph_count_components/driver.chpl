...
proc correctComponentCount(A: [] int, N: int): int {
...
proc makeGraph(ref A: [] int, N: int, rs: randomStream(int)) {
  A = 0;
  for i in 0..<N { for j in i+1..<N { if rs.getNext() % 3 == 0 { A[i*N+j] = 1; A[j*N+i] = 1; } } }
}
...
proc doValidate(): bool {
  ...
  for trial in 0..1 {
    ...
    if correctComponentCount(A, testN) != componentCount(A, testN) then return false;
  }
  return true;
}
...
proc main() {
  ...
  for it in 0..<NITER { sw.restart(); var r = componentCount(A, N); sw.stop(); computeTotal += sw.elapsed(); }
  writeln("Time: ", computeTotal / NITER);

  var bestTotal = 0.0;
  for it in 0..<NITER { sw.restart(); var r = correctComponentCount(A, N); sw.stop(); bestTotal += sw.elapsed(); }
