#!/usr/bin/env python3
"""charm4py benchmark driver for 53_stencil_2d_jacobi_5-point_stencil"""
import argparse
...
def validate(gen_fn):
    for _ in range(MAX_VALIDATION_ATTEMPTS):
        N = N_VAL
        inp = [random.uniform(-100.0, 100.0) for _ in range(N * N)]
        ref = [0.0] * (N * N)
        ...
        for i in range(1, N - 1):
            for j in range(1, N - 1):
                if abs(tst[i * N + j] - ref[i * N + j]) > 1e-6:
                    return False
    return True
...
def main(args):
    ...
    try:
        gen_fn = load_fn(opts.generated, "jacobi2D")
    except Exception:
