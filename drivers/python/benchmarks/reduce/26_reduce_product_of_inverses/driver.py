#!/usr/bin/env python3
"""charm4py benchmark driver for 26_reduce_product_of_inverses"""
...
def correct_fn(x):
    ...
    for i, v in enumerate(x):
        result *= (1.0 / v) if i % 2 else v
    return result


def validate(gen_fn):
    for _ in range(MAX_VALIDATION_ATTEMPTS):
        x = [random.uniform(1.0, 100.0) for _ in range(1024)]
...
def main(args):
    ...
    try:
        gen_fn = load_fn(opts.generated, "productWithInverses")
    except Exception:
    ...
    data = [random.uniform(1.0, 100.0) for _ in range(PROBLEM_SIZE)]
